defmodule DockdWeb.DockdComponents do
  ...
  def dockd_app_shell(assigns) do
    ...
    <div class="min-h-screen bg-base-200 text-base-content">
        ...
        <aside class="hidden w-64 shrink-0 border-r border-base-content/10 bg-base-100 p-6 lg:flex lg:flex-col">
          ...
          <nav class="mt-12 space-y-2" aria-label="Navegação principal">
            <%= for item <- navigation() do %>
              <.link navigate={item.path} class={["flex min-h-11 items-center rounded-xl px-4 text-sm font-semibold transition-colors hover:bg-base-200", item.label == @current && "bg-primary text-primary-content hover:bg-primary"]}>
                {item.label}
              </.link>
            <% end %>
          </nav>
      ...
      <nav class="fixed inset-x-0 bottom-0 z-30 border-t border-base-content/10 bg-base-100/95 px-3 py-2 backdrop-blur lg:hidden" aria-label="Navegação principal">
        <div class="mx-auto flex max-w-lg justify-around">
          <%= for item <- navigation() do %>
            <.link navigate={item.path} class={["flex min-h-11 min-w-20 flex-col items-center justify-center rounded-xl px-3 text-xs font-semibold transition-colors hover:bg-base-200", item.label == @current && "bg-primary text-primary-content hover:bg-primary"]}>
              {item.label}
            </.link>
          <% end %>
        </div>
  ...
  def theme_toggle(assigns) do
    ...
    <div class="flex items-center gap-1 rounded-xl border border-base-content/10 bg-base-200 p-1">
      <button type="button" class="min-h-9 rounded-lg px-3 text-xs font-semibold hover:bg-base-100" phx-click={JS.dispatch("phx:set-theme")} data-phx-theme="dockd-light">Claro</button>
      <button type="button" class="min-h-9 rounded-lg px-3 text-xs font-semibold hover:bg-base-100" phx-click={JS.dispatch("phx:set-theme")} data-phx-theme="dockd-dark">Escuro</button>
    </div>
    """
  end
  ...
  def page_header(assigns) do
    ...
    <div class="flex flex-wrap items-end justify-between gap-5">
      <div class="min-w-0">
        ...
        <p :if={@description} class="mt-3 max-w-2xl text-base leading-relaxed text-base-content/70">{@description}</p>
      </div>
