defmodule Dockd.Library do
  @moduledoc "User-scoped library, ownership, and release veto operations."
  import Ecto.Query
  ...
  def create_entry(%User{id: user_id}, attrs) do
    ...
    |> event_for(:added, user_id, attrs)
    |> Repo.transaction()
    |> result(:entry)
  end

  @doc "Updates an entry and records each state transition atomically."
  def update_entry(%User{id: user_id}, %Entry{} = entry, attrs) do
    if entry.user_id != user_id,
      do: {:error, :not_found},
      else: update_entry_for_user(user_id, entry, attrs)
  end

  defp update_entry_for_user(user_id, entry, attrs) do
    changeset = entry_changeset(entry, attrs)
    multi = Ecto.Multi.new() |> Ecto.Multi.update(:entry, changeset)

    multi =
      Enum.reduce([:purchase_intent, :play_state, :backlog], multi, fn field, acc ->
  ...
  def delete_entry(%User{id: user_id}, %Entry{} = entry),
    do: if(entry.user_id == user_id, do: Repo.delete(entry), else: {:error, :not_found})
  ...
  def create_ownership(%User{id: id}, attrs),
    do:
      %Ownership{user_id: id}
      |> ownership_changeset(Map.put(attrs, :user_id, id))
      |> Repo.insert()

  @doc "Gets an ownership record scoped to a user."
  def get_ownership!(%User{id: id}, ownership_id),
    do: Repo.one!(from o in Ownership, where: o.id == ^ownership_id and o.user_id == ^id)

  @doc "Updates an ownership record scoped to a user."
  def update_ownership(%User{id: id}, %Ownership{} = ownership, attrs),
    do:
      if(ownership.user_id == id,
        do: ownership |> ownership_changeset(attrs) |> Repo.update(),
        else: {:error, :not_found}
      )

  @doc "Deletes an ownership record scoped to a user."
  def delete_ownership(%User{id: id}, %Ownership{} = ownership),
    do: if(ownership.user_id == id, do: Repo.delete(ownership), else: {:error, :not_found})

  @doc "Lists release vetoes for a user."
  def list_vetoes(%User{id: id}),
    do: Repo.all(from v in ReleaseVeto, where: v.user_id == ^id, preload: [:release])
  ...
  def create_veto(%User{id: user_id}, attrs) do
    ...
    Ecto.Multi.new()
    |> Ecto.Multi.insert(:veto, cs)
    |> event_for(:vetoed, user_id, attrs)
    |> Repo.transaction()
    |> result(:veto)
  end

  @doc "Gets a release veto scoped to a user."
  def get_veto!(%User{id: id}, veto_id),
    do: Repo.one!(from v in ReleaseVeto, where: v.id == ^veto_id and v.user_id == ^id)

  @doc "Updates a release veto scoped to a user."
  def update_veto(%User{id: id}, %ReleaseVeto{} = veto, attrs),
    do:
      if(veto.user_id == id,
        do: veto |> veto_changeset(attrs) |> Repo.update(),
        else: {:error, :not_found}
      )

  @doc "Deletes a release veto scoped to a user."
  def delete_veto(%User{id: id}, %ReleaseVeto{} = veto),
    do: if(veto.user_id == id, do: Repo.delete(veto), else: {:error, :not_found})

  defp entry_changeset(entry, attrs),
    do:
      entry
      |> cast(attrs, [
        :user_id,
  ...
  defp ownership_changeset(o, attrs),
    do:
      ...
      |> unique_constraint([:user_id, :release_id, :ownership_type])

  defp veto_changeset(v, attrs),
    do:
      v
      |> cast(attrs, [:user_id, :release_id, :reason])
      |> validate_required([:user_id, :release_id])
      |> assoc_constraint(:release)
      |> unique_constraint([:user_id, :release_id])

  defp event_for(multi, type, user_id, attrs),
    do:
  ...
  defp event_type(:play_state, :unplayed), do: nil
